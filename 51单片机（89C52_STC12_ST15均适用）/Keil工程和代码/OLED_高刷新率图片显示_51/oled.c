...
{//参数：图片起始列坐标， 行坐标（页），结束列坐标，结束行坐标，  图片名，  移动方向， 移动速度
	...
	if(BMPfx == LEFT)																											//方向：左
	{
		...
		{	
			...
			OLED_DrawBMP_Faster(bmx+BMPx2,BMPy1,bmx+BMPx2+BMPx2-BMPx1,BMPy2,BMPclr);	//擦除右侧残影
			bmx -= 8;																													//左移8列像素
		}
	}
	if(BMPfx == RIGHT)																										//方向：右
	{
		...
		{
			OLED_DrawBMP_Faster(bmx+BMPx1,BMPy1,bmx+BMPx2,BMPy2,BMPn);								//按当前坐标更新图形
			OLED_DrawBMP_Faster(bmx+BMPx1,BMPy1,bmx+BMPx2+BMPx2-BMPx1,BMPy2,BMPclr);	//擦除左侧残影
			bmx +=8;																													//右移8列像素
		}
	}	
